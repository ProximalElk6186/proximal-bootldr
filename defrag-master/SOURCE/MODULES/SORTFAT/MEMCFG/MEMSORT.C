                       ...
                       int amofentries)
{
     ...
     for (; i < amofentries; i++)
	 if ((temp[i].attribute & FA_LABEL) == 0)
            realentries++;
                                          ...
                                          unsigned pos)
{
    ...
    for (i = 0; i < pos; i++)
    {
        while (pEntries->attribute & FA_LABEL)
              pEntries++;  
        pEntries++;
    }
...
static int swapentry(void* entries, unsigned pos1, unsigned pos2)
{
     ...
     SwapBufferParts(entry1, entry1+DIRLEN2BYTES(EntryLength(entry1)),
                     entry2, entry2+DIRLEN2BYTES(EntryLength(entry2)));  
